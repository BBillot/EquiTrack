...
class GatedBlock(torch.nn.Module):
    def __init__(self,
                 ...
                 activation=(None, None), stride=1, padding=0, dilation=1, capsule_dropout_p=None,
        ...
        :param int size: the filters are cubes of dimension = size * size x size
        :param radial_window: radial window function
        :param activation: (scalar activation, gate activation) which are functions torch.nn.functional.relu or None
        ...
        :param bool dyn_iso: initialize with some sort of Dynamical Isometry (https://arxiv.org/abs/1806.05393)
        """
        super().__init__()
        ...
        Rs_in = [(m, l) for l, m in enumerate(repr_in)]              # list of [(n_features, order)]
        Rs_out_with_gate = [(m, l) for l, m in enumerate(repr_out)]  # list of [(n_features, order)]

        # activation for the first order channels
        ...
        # activation for the channels with order > 0
        ...
        if normalization is None:
            ...
            Convolution = partial(SE3GNConvolution,
                                  Rs_gn=[(1, 2 * n + 1) for n, mul in enumerate(repr_in) for _ in range(mul)])
        else:
        ...
        self.conv = Convolution(Rs_in=Rs_in,
    ...
    def forward(self, x):  # pylint: disable=W
        ...
        if self.scalar_act is not None or self.gate_act is not None:
            z = torch.utils.checkpoint.checkpoint(self.gate, z) if self.checkpoint else self.gate(z)
