...
Therefore
    ...
    K(0, g |x| e) = R_out(g) K(0, |x| e) R_in(g^{-1})  where e is a preferred chosen unit vector and g is in SO(3)
"""
import torch
...
def _sample_sh_cube(size, J, version=3):  # pylint: disable=W0613
    ...
    No band-limiting considered, aliased regions need to be cut by windowing!
...
def _sample_cube(size, order_in, order_out):
    ...
    # r_field = radius_filed = distance to center of cube of size (size, size, size)
...
def cube_basis_kernels(size, order_in, order_out, radial_window):
    ...
    :param radial_window: callable for windowing out radial parts, taking mandatory parameters 'solutions', 'r_field'
    and 'order_irreps'
...
def gaussian_window_wrapper(solutions, r_field, order_irreps, mode='compromise', border_dist=0., sigma=.6):
    ...
    :param mode: string in ['sfcnn', 'conservative', '']
                 'conservative': radial bandlimits such that equivariance value stays over 90% (for border_dist=0 and
                                 except for outer shell)
                          [0,2,4,6,8,10,...]
...
class SE3Kernel(torch.nn.Module):
        ...
        for i, (m_out, l_out) in enumerate(self.Rs_out):
            for j, (m_in, l_in) in enumerate(self.Rs_in):
                ...
                if basis is not None:
                    assert basis.size()[1:] == ((2 * l_out + 1), (2 * l_in + 1), size, size, size), \
                        "wrong basis shape - your cache files may probably be corrupted"

                    if verbose:
                        overlaps = torch.stack([check_basis_equivariance(basis, l_in, l_out, a, b, c)
                                                for a, b, c in torch.rand(5, 3)]).mean(0)
                        print("{} -> {} : Created {} basis elements with equivariance {}".format(
                            l_in, l_out, len(basis), overlaps))
                    ...
                    # BB: we do this one
                        ...
                        weights += [torch.randn(m_out * m_in * basis.size(0))]  # last is batch size
    ...
    def __repr__(self):
        return "{name} ({Rs_in} -> {Rs_out}, size={size})".format(name=self.__class__.__name__,
                                                                  Rs_in=self.Rs_in,
                                                                  Rs_out=self.Rs_out,
                                                                  size=self.size)
        ...
        for i, mi in enumerate(self.multiplicities_out):
            ...
            for j, mj in enumerate(self.multiplicities_in):
                ...
                if kij is not None:
                    ...
                    ker = torch.zeros(i_diff, j_diff, self.size, self.size, self.size,
